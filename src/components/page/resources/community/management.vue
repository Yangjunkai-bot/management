<template>
  <div>
    ...
    <el-dialog title="置顶"
               :show-close='false'
               :close-on-click-modal='false'
               :visible.sync="isPlacedVisible"
               width="400px">
      <el-form ref="isPlacedFormRules"
               :rules="isPlaceRules"
               :model="isPlacedform"
               label-width="120px">
        <el-form-item label="置顶结束时间"
                      prop="topEndTime">
          <!-- <el-date-picker v-model="isPlacedform.topEndTime"
      ...
      <span slot="footer"
            class="dialog-footer">
        <el-button @click="isPlacedVisible = false">取 消</el-button>
        <el-button type="primary"
                   @click="isPlaced">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    handleIsPlaced (row) {
      if (row.top === 0) {
        updateTopStatus({ 'id': row.id, 'top': '1' }).then((res) => {
          if (res.data.code === 0) {
            this.getData()
          } else {
            this.$message({
    ...
    isPlaced () {
      this.$refs.isPlacedFormRules.validate((valid) => {
        if (valid) {
          this.isPlacedVisible = false
          updateTopStatus({ 'id': this.isPlacedform.id, 'top': '0', 'topEndTime': this.isPlacedform.topEndTime / 1000 }).then((res) => {
            if (res.data.code === 0) {
              this.getData()
            } else {
              this.$message({
